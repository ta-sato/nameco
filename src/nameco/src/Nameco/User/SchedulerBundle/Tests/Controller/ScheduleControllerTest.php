...
class ScheduleControllerTest extends BaseSchedulerControllerTest
{
	...
	public function setUp()
	{
		...
		$this->_loginUser = $this->_em->getRepository('NamecoUserSchedulerBundle:User')->findOneByUsername("test0");
	}
	...
	public function testMonth()
	{
		...
		$repo = $em->getRepository('NamecoUserSchedulerBundle:User');
	...
	public function testNew()
	{
		...
		$id = $user->getId();
		$id2 = $em->getRepository('NamecoUserSchedulerBundle:User')->findOneByUsername('test1')->getId();
		
		$est = $em->getRepository('NamecoUserSchedulerBundle:Establishment')->findOneByName('Establishment_0');
		$estId = $est->getId();
	...
	public function testCreate()
	{
		...
		$est = $em->getRepository('NamecoUserSchedulerBundle:Establishment')->findOneByName('Establishment_0');
		$estId = $est->getId();
		$beforeCount = count($em->getRepository('NamecoUserSchedulerBundle:Schedule')->findAll());
		...
		$this->assertGreaterThan($beforeCount, count($em->getRepository('NamecoUserSchedulerBundle:Schedule')->findAll()));
	...
	public function testGetUsers()
	{
		...
		$data = $serializer->deserialize($client->getResponse()->getContent(), 'ArrayCollection<Nameco\User\SchedulerBundle\Entity\User>', 'json');
